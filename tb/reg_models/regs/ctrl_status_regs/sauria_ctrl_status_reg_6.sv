// Stalls Counter register (offset 0x18)
class sauria_ctrl_status_reg_6 extends uvm_reg;

    `uvm_object_utils(sauria_ctrl_status_reg_6)

    uvm_reg_field stalls_counter;

    function new(string name="sauria_ctrl_status_reg_6");
        super.new(name, SAURIA_REG_SIZE, REG_COVERAGE);
    endfunction

    function void build_fields();
        stalls_counter = uvm_reg_field::type_id::create("ctrl_status_stalls_counter");
    endfunction
    ...
    function void configure(uvm_reg_block blk_parent, uvm_reg_file regfile_parent=null, string hdl_path="");
        ...
        // 32-bit counter: stall cycles in the previous computation
        stalls_counter.configure(.parent(this),
                     .size(CTRL_STATUS_STALLS_COUNTER_SIZE),
                     .lsb_pos(CTRL_STATUS_STALLS_COUNTER_LSB),
