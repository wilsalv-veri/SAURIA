// Cycle Counter register (offset 0x14)
class sauria_ctrl_status_reg_5 extends uvm_reg;

    `uvm_object_utils(sauria_ctrl_status_reg_5)

    uvm_reg_field cycle_counter;

    function new(string name="sauria_ctrl_status_reg_5");
        super.new(name, SAURIA_REG_SIZE, REG_COVERAGE);
    endfunction

    function void build_fields();
        cycle_counter = uvm_reg_field::type_id::create("ctrl_status_cycle_counter");
    endfunction
    ...
    function void configure(uvm_reg_block blk_parent, uvm_reg_file regfile_parent=null, string hdl_path="");
        ...
        // 32-bit counter: total cycles spent in the previous computation
        cycle_counter.configure(.parent(this),
                    .size(CTRL_STATUS_CYCLE_COUNTER_SIZE),
                    .lsb_pos(CTRL_STATUS_CYCLE_COUNTER_LSB),
