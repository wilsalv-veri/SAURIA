class sauria_core_ctrl_status_reg_1 extends uvm_reg;

    `uvm_object_utils(sauria_core_ctrl_status_reg_1)

    uvm_reg_field global_interrupt_en;

    function new(string name="sauria_core_ctrl_status_reg_1");
        super.new(name, SAURIA_REG_SIZE, REG_COVERAGE);
    endfunction

    function void build_fields();
        global_interrupt_en = uvm_reg_field::type_id::create("core_ctrl_status_global_interrupt_en");
    endfunction
    ...
    function void configure(uvm_reg_block blk_parent, uvm_reg_file regfile_parent=null, string hdl_path="");
        ...
        global_interrupt_en.configure(.parent(this),
                          .size(CORE_CTRL_STATUS_GLOBAL_INT_EN_SIZE),
                          .lsb_pos(CORE_CTRL_STATUS_GLOBAL_INT_EN_LSB),
