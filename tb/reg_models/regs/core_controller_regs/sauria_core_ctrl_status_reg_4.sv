class sauria_core_ctrl_status_reg_4 extends uvm_reg;

    `uvm_object_utils(sauria_core_ctrl_status_reg_4)

    uvm_reg_field status_flags;

    function new(string name="sauria_core_ctrl_status_reg_4");
        super.new(name, SAURIA_REG_SIZE, REG_COVERAGE);
    endfunction

    function void build_fields();
        status_flags = uvm_reg_field::type_id::create("core_ctrl_status_status_flags");
    endfunction
    ...
    function void configure(uvm_reg_block blk_parent, uvm_reg_file regfile_parent=null, string hdl_path="");
        ...
        status_flags.configure(.parent(this),
                               .size(CORE_CTRL_STATUS_STATUS_FLAGS_SIZE),
                               .lsb_pos(CORE_CTRL_STATUS_STATUS_FLAGS_LSB),
