class sauria_axi4_lite_driver extends uvm_driver #(sauria_axi_txn_base_seq_item);
    ...
    sauria_axi_txn_base_seq_item      axi_txn_base_seq_item;
    ...
    virtual function void build_phase(uvm_phase phase);
        ...
        axi_txn_base_seq_item = sauria_axi_txn_base_seq_item::type_id::create("sauria_axi_txn_base_seq_item");
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        forever begin
            seq_item_port.get_next_item(axi_txn_base_seq_item);
            
            case(axi_txn_base_seq_item.txn_type)
                RD_TXN : drive_axi4_lite_cfg_rd_txn(axi_txn_base_seq_item);
                WR_TXN : drive_axi4_lite_cfg_wr_txn(axi_txn_base_seq_item);
            endcase 
    ...
    virtual task drive_axi4_lite_cfg_rd_txn(sauria_axi_txn_base_seq_item txn_seq_item);
        ...
        else `sauria_error(message_id, "Failed to cast axi_txn_base_seq_item into axi4_lite_rd_txn_seq_item")
        
    endtask
    
    virtual task drive_axi4_lite_cfg_wr_txn(sauria_axi_txn_base_seq_item txn_seq_item);
        if ($cast(cfg_wr_txn_item, txn_seq_item)) begin
        
            fork 
                drive_wr_addr_ch(cfg_wr_txn_item.wr_addr_item);
                drive_wr_data_ch(cfg_wr_txn_item.wr_data_item);
                drive_wr_rsp_ch();
            join

            case(cfg_wr_txn_item.wr_rsp_item.bresp)
                SLVERR: `sauria_error(message_id, "The slave successfully received the transaction address and data, but it encountered an error when attempting to perform the write operation")
                DECERR: `sauria_error(message_id, "The address was not mapped to any peripheral (slave)")
            endcase
        end
        else `sauria_error(message_id, "Failed to cast axi_txn_base_seq_item into axi4_lite_wr_txn_seq_item")
    ...
    virtual task drive_wr_rsp_ch();
        ...
        cfg_wr_txn_item.wr_rsp_item.bresp  = sauria_axi4_lite_cfg_if.axi4_lite_wr_rsp_ch.bresp;
        cfg_wr_txn_item.wr_rsp_item.bvalid = sauria_axi4_lite_cfg_if.axi4_lite_wr_rsp_ch.bvalid;
    endtask
