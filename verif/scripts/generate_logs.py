...
SAURIA = os.environ["SAURIA"]
dsim_log = "dsim.log"
test_runs_path = f"{SAURIA}/test_runs"
cwd_path = os.getcwd()
    
class SAURIA_log:

    def __init__(self,name, match_string):
        self.name         = SAURIA_log.get_formated_name(name)
        self.lines        = []
...
def generate_logs(test_run_path):
    ...
    for key in func_log_keys:
        logs[key] = SAURIA_log(key.lower(), key)
    
    
    logs["PASS_FAIL"] = SAURIA_log("PASS_FAIL".lower(), "PASS_FAIL")

    filepath = f"{SAURIA}/output/{dsim_log}"
    try:
    ...
    for key in logs:
        ...
        if key == "PASS_FAIL":
            ...
            new_formated_name = SAURIA_log.get_formated_name(new_name[new_name_idx].lower())
